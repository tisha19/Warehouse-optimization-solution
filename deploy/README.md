...
Run this **on your own machine**, not on the cluster â€” SSH port forwarding is
...
| Lightning NIM | <http://127.0.0.1:28000> |
...
|---|---|---|
| `runall.sh` | login node | Status â†’ start what is missing â†’ follow logs â†’ final status â†’ URL. The normal entry point. |
...
| `hosted_ultra_test.py` | anywhere with `.env` | Four checks against a hosted model â€” model list, completion, tool calling, streamed reasoning. Takes an optional model id. |
...
| `guardrails_warehouse_config.json` | â€” | The rails config. `cluster_up.sh` rewrites its model id to the specialist model in use. |
| `openshell/` | â€” | Governor policy and sandbox definitions. |
...
| UI code changed | Rebuild the bundle, then `srun --jobid=<ID> --overlap pkill -f 'showcase_server\.py'` â€” the monitor restarts it within ~60s |
...
limit cannot be raised by its owner** â€” `scontrol update` returns
...
- **Rootless docker**: `/cm/shared/apps/rootless-docker/bin`, per-user socket at
  ...
  itself â€” never from a transient `srun --overlap` step.
...
benign input gets blocked. `guardrails_warehouse_config.json` passes
`extra_body.chat_template_kwargs.enable_thinking = false`; nested under
`extra_body` because the OpenAI SDK rejects unknown top-level arguments. With it
the rail answers `no` in two tokens instead of 740.
