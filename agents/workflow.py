...
from services.nvidia import CuOptClient, GuardrailsClient, NIMClient, NeMoRetriever
...
class ProductionWarehouseWorkflow:
    ...
    def __init__(self, config: ProductionConfig | None = None, specialist_runner: Callable[[Dict[str, Any]], Dict[str, Any]] | None = None, wms: WMSAdapter | None = None, erp: ERPAdapter | None = None, forecast: ForecastAdapter | None = None):
        ...
        self.retriever = NeMoRetriever(self.config) if self.config.retriever_url else None
    ...
    def build_graph(self):
        ...
        graph.add_edge("ingest", "retrieve_policies")
        graph.add_edge("retrieve_policies", "plan")
    ...
    def _retrieve_policies(self, state: WarehouseState) -> Dict[str, Any]:
        self._trace(state, "retrieve_policies", "Retrieving warehouse policies and operating procedures")
        documents = self.retriever.search(state["business_goal"], 8) if self.retriever else []
        return {"policy_documents": documents}
    ...
    def _optimize(self, state: WarehouseState) -> Dict[str, Any]:
        ...
        if not self.cuopt:
            raise RuntimeError("CUOPT_URL is required; heuristic optimization is not used in production workflow")
